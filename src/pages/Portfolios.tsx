  ...
  return (
    <div className="space-y-10 animate-fade-in pb-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-2 bg-white/10 dark:bg-white/5 p-1.5 rounded-2xl border border-white/10">
          ...
          <button className="px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest text-white/40 hover:text-white transition-all">Grille</button>
        </div>
      </div>

      {/* Sound Console Layout */}
      <div className="flex flex-col lg:flex-row gap-8 min-h-[500px] lg:min-h-[650px]">
        {/* Channel Strips */}
        <div className="flex-1 flex gap-5 overflow-x-auto pb-8 no-scrollbar snap-x snap-mandatory">
          {portfolios.map((p, index) => (
            <motion.div
              ...
              className={`relative flex-shrink-0 w-32 md:w-40 h-[450px] md:h-[600px] rounded-[3rem] cursor-pointer transition-all duration-500 overflow-hidden border-2 snap-center ${
                selectedId === p.id 
                  ? 'border-electric bg-cream dark:bg-slate-900 shadow-2xl shadow-electric/20 ring-4 ring-electric/5' 
                  : 'border-white/5 bg-white/5 dark:bg-slate-900/40 hover:bg-white/10 shadow-sm'
              }`}
              aria-label={`Canal ${index + 1}: ${p.name}`}
            >
              {/* Channel Label */}
              <div className="absolute top-10 left-1/2 -translate-x-1/2 -rotate-90 origin-center whitespace-nowrap">
                <span className={`text-[10px] font-black uppercase tracking-[0.4em] transition-colors ${selectedId === p.id ? 'text-electric' : 'text-white/20 dark:text-slate-600'}`}>
                  CH-{String(index + 1).padStart(2, '0')}
                </span>
              </div>

              {/* Fader Track */}
              <div className="absolute inset-y-20 left-1/2 -translate-x-1/2 w-2 bg-navy/20 dark:bg-white/10 rounded-full shadow-inner">
                {/* Fader Knob */}
                <motion.div 
                  animate={{ top: selectedId === p.id ? '25%' : '75%' }}
                  className={`absolute left-1/2 -translate-x-1/2 w-12 h-20 rounded-2xl shadow-2xl border-2 flex flex-col items-center justify-center gap-2 transition-colors ${
                    selectedId === p.id 
                      ? 'bg-electric border-electric text-white' 
                      : 'bg-cream dark:bg-slate-700 border-white/10 text-slate-400 dark:text-slate-500'
                  }`}
                >
                  <div className={`w-6 h-1 rounded-full ${selectedId === p.id ? 'bg-white/50' : 'bg-slate-200 dark:bg-slate-600'}`} />
                  <div className={`w-6 h-1 rounded-full ${selectedId === p.id ? 'bg-white/50' : 'bg-slate-200 dark:bg-slate-600'}`} />
                  <div className={`w-6 h-1 rounded-full ${selectedId === p.id ? 'bg-white/50' : 'bg-slate-200 dark:bg-slate-600'}`} />
                </motion.div>
              </div>
              ...
              {/* Level Meter */}
              <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col gap-1.5">
                {[...Array(14)].map((_, i) => (
                  <motion.div
                    key={i}
                    animate={{ 
                      opacity: selectedId === p.id ? (i < 10 ? 1 : 0.4) : (i < 4 ? 0.8 : 0.1),
                      backgroundColor: i > 11 ? '#EF4444' : i > 9 ? '#F59E0B' : '#10B981',
                      scale: selectedId === p.id && i < 10 ? [1, 1.2, 1] : 1
                    }}
              ...
              {/* Vertical Title */}
              <div className="absolute bottom-36 left-1/2 -translate-x-1/2 -rotate-90 origin-center whitespace-nowrap">
                <span className={`text-sm font-black uppercase tracking-[0.2em] transition-colors ${selectedId === p.id ? 'text-electric' : 'text-white/60 dark:text-white'}`}>
                  {p.name}
                </span>
              </div>
            </motion.div>
        ...
        {/* Details Panel - The "Synthesizer" Interface */}
        <AnimatePresence mode="wait">
          {selectedPortfolio ? (
            <motion.div
              ...
              className="w-full lg:w-[520px] bg-cream dark:bg-slate-900 rounded-[3.5rem] p-12 shadow-2xl border border-white/10 relative overflow-hidden"
            >
              {/* Decorative Background Elements */}
              <div className="absolute top-0 right-0 w-80 h-80 bg-electric/5 rounded-full blur-[100px] -mr-40 -mt-40" />
              ...
              <div className="relative z-10 space-y-10">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <h2 className="text-4xl font-black text-navy dark:text-white tracking-tighter">{selectedPortfolio.name}</h2>
                ...
                <div className="p-10 bg-navy/5 dark:bg-white/5 rounded-[2.5rem] border border-white/10 space-y-8">
                  ...
                  <div className="h-20 flex items-center justify-between gap-1.5 px-2">
                    {[...Array(30)].map((_, i) => (
                      <motion.div
                        key={i}
                        animate={{ height: [10, Math.random() * 50 + 10, 10] }}
                        transition={{ duration: 0.5, repeat: Infinity, delay: i * 0.05 }}
                        className="flex-1 bg-electric/30 rounded-full"
                      />
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {selectedPortfolio.tags.map(tag => (
                      <span key={tag} className="px-4 py-2 bg-white dark:bg-white/5 text-slate-600 dark:text-slate-300 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-slate-100 dark:border-white/10 shadow-sm">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex items-center justify-between pt-10 border-t border-white/10">
                  <div className="font-black text-4xl text-navy dark:text-white tracking-tighter">
            ...
            <div className="w-full lg:w-[520px] flex flex-col items-center justify-center text-center p-16 bg-white/5 dark:bg-slate-900/50 rounded-[3.5rem] border-4 border-dashed border-white/5">
              <div className="w-28 h-28 bg-white/10 dark:bg-white/5 rounded-[2.5rem] flex items-center justify-center text-white/20 dark:text-slate-700 mb-10 shadow-inner">
                <Waveform size={56} />
              </div>
              <h3 className="text-3xl font-black text-white uppercase tracking-wider">Sélectionnez un canal</h3>
              <p className="text-base text-white/40 mt-4 font-medium max-w-[280px] leading-relaxed">Cliquez sur une tranche de console pour voir les détails du portefeuille.</p>
            </div>
          )}
        </AnimatePresence>
      </div>
